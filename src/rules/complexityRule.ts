            ...
            if (node.type === 'method_declaration') {
                ...
                function calculate(childNode: any, depth: number) {
                    ...
                    if (childNode.type === 'binary_expression') {
                        const ops = (childNode.text.match(/&&|\|\|/g) || []).length;
                        cognitiveScore += ops;
                    }
                ...
                if (cognitiveScore > 10) {
                    violations.push({
                        ...
                        message: `[Violation : High Complexity] Method '${methodName}' memiliki kompleksitas tinggi (score: ${cognitiveScore}).`,
                    ...
                    violations.push({
                        node: methodNameNode || node,
                        message: `[Violation : High Complexity] Method '${methodName}' mulai sulit dipahami (score: ${cognitiveScore}).`,
